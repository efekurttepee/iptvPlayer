  ...
  return (
    <>
      ...
      {upToDateInfo && !updateAvailable && (
        <div className="fixed top-5 right-5 z-50 flex items-center space-x-3 bg-gradient-to-r from-[#0d1f18]/95 to-[#0b141b]/95 backdrop-blur-xl border border-emerald-500/40 px-4 py-3 rounded-2xl shadow-2xl shadow-emerald-500/20 animate-fadeIn pointer-events-auto max-w-sm">
          ...
          <div className="flex-1 pr-2">
            <div className="flex items-center space-x-1.5">
              ...
              <span className="text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-1.5 py-0.2 rounded-md">
                v{upToDateInfo.version}
              </span>
            </div>
            <p className="text-[11px] text-gray-300 mt-0.5">En son IPTV Player Pro sürümünü kullanıyorsunuz.</p>
          </div>
          <button
            onClick={() => setUpToDateInfo(null)}
            className="w-6 h-6 rounded-full bg-white/5 hover:bg-white/15 text-gray-400 hover:text-white flex items-center justify-center transition-colors"
          >
